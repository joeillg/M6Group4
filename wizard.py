...
class Wizard:
    ...
    def __init__(self,name):  # Testable (range of hitpoints, and hitpoints == max hitpoints)
        self.d100 = Die(100)
        self.d20 = Die(20)
        self.d10 = Die(10)
        self.d6 = Die(6)
        self.attackChoices = ["Fireball", "Disintegrate", "Avada Kedavra","Self Heal"]
        if name != "AI":
            self.name = name
        else:
            self.name = "Wizard"
    ...
    def attack(self, attack_type:int) -> int:  # Testable. You could do some range tests with damage
        ...
        if (attack_type == 1): # Fireball
            if (self.d20.roll() >= 13):  # do we hit?
                ...
                print(f"{self.name} Fireball hits for {damage}")
            else:
                print(f"{self.name} Fireball missed.")

        elif (attack_type == 2): #Disintegrate
            if (self.d20.roll() >= 16):
                damage = self.d6.roll() + self.d6.roll() + self.d6.roll()  # 3d6
                print(f"{self.name} Disintegrate hits for {damage}")
            else:
                print(f"{self.name} Disintegrate missed.")
            ...
            if self.special_attack_count == 5:
                damage = 50
                self.special_attack_count = 0
                print(f"{self.name} Avada Kedavra for {damage}")
            else:
                print(f'Avada Kedavra incantation is {int(self.special_attack_count*100/5)}% charged!')
        else: #Self Heal
            damage = -1 * self.d6.roll()
            print(f" {self.name} heals for {-1*damage} damage")
